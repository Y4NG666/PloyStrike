...
const pending = new Map<string, { price: number; since: number }>();
const lastSent = new Map<string, number>();

async function loadVwap(skinId: string, source: string): Promise<number | null> {
...
function shouldSubmitPrice(skinId: string, price: number): boolean {
  const now = Date.now();
...
async function processPrices(relayer: OracleRelayer) {
...
async function runOnce(relayer: OracleRelayer) {
