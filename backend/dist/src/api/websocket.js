...
export function setupWebSocket(server) {
    ...
    const interval = setInterval(async () => {
        ...
        const unboxPayload = unboxSnapshot
            ...
            : null;
        const now = Date.now();
        const createdRecently = unboxSnapshot && now - new Date(unboxSnapshot.createdAt).getTime() <= env.wsIntervalMs * 1.5;
        const sessionCreatedPayload = createdRecently
            ...
            : null;
        const sessionBettingPayload = unboxSnapshot?.status === "BETTING"
            ? JSON.stringify({
            ...
            : null;
        const sessionResolvedPayload = unboxSnapshot && ["RESOLVED", "REFUNDABLE"].includes(unboxSnapshot.status)
            ? JSON.stringify({
